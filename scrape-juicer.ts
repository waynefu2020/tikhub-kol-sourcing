...
// === 产品配置 ===
const PRODUCT = 'masticating-juicer';
const CATEGORY_KEYWORDS = ['juicer', 'juice', 'juicing', 'cold press', 'masticating', 'blender', 'smoothie'];
const COMPETITOR_KEYWORDS = ['hurom', 'omega juicer', 'kuvings', 'breville'];
...
const SEARCH_TASKS = [
  ...
  { keyword: 'juice cleanse', dimension: 'scene' as const },
...
async function searchByVideos(): Promise<Map<string, Creator>> {
  ...
  for (const task of SEARCH_TASKS) {
    if (creatorMap.size >= TARGET_TOTAL * 3) break;
...
async function enrichWithProfile(creatorMap: Map<string, Creator>): Promise<void> {
  ...
  let enriched = 0, emailFound = 0, bioLinkFound = 0;
...
function toHTMLReport(
  creators: any[], kwStats: Map<string, number>, csvPath: string, timestamp: string
): string {
...
<script>
let currentTier='';function filterRows(){const q=document.getElementById('search').value.toLowerCase();const rows=document.querySelectorAll('#tableBody tr');let visible=0;rows.forEach(row=>{const tier=row.getAttribute('data-tier');const search=row.getAttribute('data-search');const match=(!currentTier||tier===currentTier)&&(!q||search.includes(q));row.style.display=match?'':'none';if(match)visible++});document.getElementById('emptyState').style.display=visible===0?'':'none'}
function filterByTier(tier){currentTier=tier;document.querySelectorAll('.toolbar button').forEach(b=>b.classList.remove('active'));event.target.classList.add('active');filterRows()}
let sortDir=1;function sortTable(col){const tbody=document.getElementById('tableBody');const rows=Array.from(tbody.querySelectorAll('tr'));rows.sort((a,b)=>{let av=a.cells[col].textContent.trim();let bv=b.cells[col].textContent.trim();const an=parseFloat(av.replace(/[^0-9.]/g,''));const bn=parseFloat(bv.replace(/[^0-9.]/g,''));if(!isNaN(an)&&!isNaN(bn))return sortDir*(an-bn);return sortDir*av.localeCompare(bv)});rows.forEach(r=>tbody.appendChild(r));sortDir*=-1}
...
async function main() {
  ...
  // 统计
  const aCount = scored.filter(c => c.tier === 'A').length;
  const bCount = scored.filter(c => c.tier === 'B').length;
  const cCount = scored.filter(c => c.tier === 'C').length;
  const emailCount = scored.filter(c => c.email).length;
  const bioLinkCount = scored.filter(c => c.bio_link).length;
  ...
  scored.forEach(c => { kwStats.set(c.search_keyword, (kwStats.get(c.search_keyword) || 0) + 1); });
  writeFileSync(htmlFilename, toHTMLReport(scored, kwStats, filename, ts), 'utf8');
  ...
  writeMetaJson(filename, {
    ...
    emailCount,
    emailRate: scored.length > 0 ? Math.round(emailCount / scored.length * 100) : 0,
    bioLinkCount,
  ...
  console.log(`📋 A级${aCount}位 · B级${bCount}位 · C级${cCount}位`);
  console.log(`📧 有邮箱：${emailCount} 位（${Math.round(emailCount / scored.length * 100)}%）`);
  console.log(`🔗 有 bioLink：${bioLinkCount} 位（${Math.round(bioLinkCount / scored.length * 100)}%）`);
